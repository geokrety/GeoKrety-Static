...
sorttable = {
  ...
  makeSortable: function(table) {
    ...
    for (var i=0; i<headrow.length; i++) {
      ...
      if (!headrow[i].className.match(/\bst_no\b/)) { // skip this col
        ...
        if (mtch) { override = mtch[1]; }
          ...
          this.appendChild(sortfwdind);
	        ...
	        for (var j=0; j<row_array.length; j++) {
				if(j % 2)
				row_array[j][1].className = 'mg1';
				else
				row_array[j][1].className = 'mg0';
  ...
  getInnerText: function(node) {
    ...
    if (node.getAttribute("data-sort") != null) {
  ...
  reverse: function(tbody) {
	...
	if(i % 2) {
		if(newrows.length % 2) //odd number of rows so start with tRowB
			newrows[i].className = 'mg1';
		else
			newrows[i].className = 'mg0';
	}
	else {
